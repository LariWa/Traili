    ...
    <div v-if="results">
      ...
      <div v-for="(result, idx) in this.promiseState.data" :key="idx">
        <v-card elevation="2" @click="setCurrentTour(result.id)">
          <v-img
                 :src="result.primaryImage ? 'http://img.oastatic.com/img/' + result.primaryImage.id + '/.jpg' : 'https://picsum.photos/id/600/1000/300?grayscale'"
...
<script>
  ...
  export default {
    ...
    watch: {
      results() {
        if (this.results) {
          resolvePromise(getHikeDetails(this.results.data.map(item => item.id)), this.promiseState, null)
        }
      }
    },
    methods: {
